...
class LicenseGuard:
    def __init__(self, server_url, product_id, allow_offline=False, license_file=None):
        self.product_id = product_id
        # allow_offline: Deprecated - Không còn hỗ trợ offline mode
        self.hwid = get_hwid()
        lic_path = license_file or f"{product_id}.lic"
        self.storage = LicenseStorage(lic_path)
    ...
    def check_license(self):
        ...
        try:
            ...
            if data.get("ok"):
                if data.get("product_id") and str(data.get("product_id")) != str(self.product_id):
                    return {
                        "ok": False,
                        "reason": "product_mismatch",
                        "message": self._get_msg("license_not_found"),
                    }
    ...
    def activate(self, license_key):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('ok'):
                    if data.get("product_id") and str(data.get("product_id")) != str(self.product_id):
                        return False, self._get_msg("license_not_found")
    ...
    def _validate_cache(self, data):
        ...
        try:
            ...
            # 3. Phải có phản hồi server hợp lệ (chống forge cache trong .lic)
            server_data = data.get("server_data", {})
            if not isinstance(server_data, dict) or server_data.get("ok") is not True:
                return False
            cached_product = server_data.get("product_id")
            if cached_product and str(cached_product) != str(self.product_id):
                return False

            # 4. Kiểm tra expire_at từ server (tránh dùng license đã hết hạn)
